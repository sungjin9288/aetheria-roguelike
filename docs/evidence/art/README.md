...
- `node --import tsx --test tests/art-asset-contract.test.js` — RED first for the absent catalog module, then GREEN with catalog, manifest, PNG IHDR, alpha, transparent-pixel, bounds, foot-baseline, SHA-256, stable-report, and fail-closed Pillow coverage.
- `npm run art:catalog` — GREEN; prints the deterministic identity above.
- `node --import tsx --test tests/art-asset-contract.test.js tests/item-visuals.test.js` — GREEN (`28/28`).
- `npm run type-check`, `npm run lint`, and `npm run verify` — GREEN (`npm run verify` unit suite: `3492/3492`).

## Intentional full-verifier RED diagnostic

`npm run art:verify` exited `1`. This is the expected Task 2 baseline while Tasks 3–8 have not supplied canonical character exports and style-version closure.
...
No `art-contract-report.json` was created or approved. The verifier refuses `--write-report` when `ok` is false; a tracked JSON report becomes evidence only after a fully green verification.
