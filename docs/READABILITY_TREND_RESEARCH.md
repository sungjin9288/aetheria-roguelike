...
- Slice 26: Commerce And Equipment Language. Shop, equipment, and inventory now share direct Korean for gold, grade, attack, defense, critical chance, life, energy, and weapon handedness. The 390px equipment summary uses stable rows instead of six narrow cards, and smoke rejects the previous mixed currency and stat abbreviations on the rendered surfaces.
...
- Open the shop, equipment, and inventory in sequence; confirm prices use `골드`, comparisons use `공격력 / 방어력 / 치명타 / 기력`, and the equipped-slot rows remain readable without legacy `CR / G / T / 1H / 2H` labels.
