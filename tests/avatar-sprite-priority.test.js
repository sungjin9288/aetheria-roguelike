...
/**
 * Avatar sprite priority — canonical job-only contract.
 *
 * 사용자 핵심 통찰: "장비를 교체했을때 아바타가 바뀌는건 직업이 바뀌는게 되는거"
 * → sprite는 오직 직업(전직)만이 결정. armor/weapon 변경은 sprite에 영향 X.
 *
 * Canonical jobs resolve to exactly one manifest path. Only malformed legacy
 * job data may use the old adventurer safe placeholder.
 *
...
test('전사는 항상 canonical warrior', () => {
    ...
    assert.equal(a, 'warrior');
    assert.equal(b, 'warrior');
});

test('어쌔신은 항상 canonical assassin', () => {
    ...
    assert.equal(a, 'assassin');
    assert.equal(b, 'assassin');
});

test('아크메이지는 항상 canonical archmage', () => {
    ...
    assert.equal(a, 'archmage');
    assert.equal(b, 'archmage');
});
...
test('그림자 주군은 정규화된 jobSlug로 매핑', () => {
    ...
    assert.equal(sprite, 'shadow-lord');
});
