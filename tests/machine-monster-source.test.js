...
const script = 'scripts/art_sources/monsters/v10/prepare.py';
function runPython(body) {
    assert.ok(existsSync(script), 'source-pinned machine preparation is required');
    const result = spawnSync('python3', ['-c', body], { encoding: 'utf8' });
    assert.equal(result.status, 0, result.stderr);
}

test('nine machine exports preserve masters, aspect ratio, binary alpha and deterministic bytes', () => {
    runPython(`
import hashlib, json, runpy, tempfile
from pathlib import Path
from PIL import Image
prepare = runpy.run_path('${script}')['prepare']
source = Path('scripts/art_sources/monsters/v10/masters')
before = {p.name: p.read_bytes() for p in source.glob('*.png')}
names = {'steel-automaton','overloaded-artillery','mummy','steam-golem','malfunctioning-robot','biological-weapon','contaminated-researcher','closed-circuit-mage-soldier','runaway-automaton'}
with tempfile.TemporaryDirectory(prefix='aetheria-machine-test-') as directory:
    base = Path(directory)
    ...
    assert Image.open(source/'contaminated-researcher.png').size == (1240,1269)
assert before == {p.name: p.read_bytes() for p in source.glob('*.png')}
`);
});

test('machine preparation refuses existing output and source drift before writing any output', () => {
    runPython(`
import runpy, shutil, tempfile
from pathlib import Path
prepare = runpy.run_path('${script}')['prepare']
source = Path('scripts/art_sources/monsters/v10/masters')
with tempfile.TemporaryDirectory(prefix='aetheria-machine-reject-') as directory:
    base = Path(directory)
    ...
    with (base/'drift'/'contaminated-researcher.png').open('ab') as file: file.write(b'drift')
    try:
