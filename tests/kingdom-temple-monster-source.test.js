...
const script = 'scripts/art_sources/monsters/v18/prepare.py';
function python(body) {
    assert.ok(existsSync(script), 'reviewed kingdom-temple source preparer is required');
    const result = spawnSync('python3', ['-c', body], { encoding: 'utf8' });
    assert.equal(result.status, 0, result.stderr);
}

test('kingdom-temple exports preserve originals and aspect ratio with deterministic bounded binary alpha', () => {
    python(`
import hashlib, json, runpy, tempfile
from pathlib import Path
from PIL import Image
prepare = runpy.run_path('${script}')['prepare']
source = Path('scripts/art_sources/monsters/v18/masters')
before = {p.name:p.read_bytes() for p in source.glob('*.png')}
names = {'golden-kingdom-guardian','kingdom-knight','mercenary-warrior','sacred-officiant','time-fragment'}
with tempfile.TemporaryDirectory(prefix='aetheria-kingdom-temple-test-') as directory:
    base=Path(directory)
...
test('kingdom-temple preparation refuses existing output and source drift before creating files', () => {
    python(`
import runpy, shutil, tempfile
from pathlib import Path
prepare=runpy.run_path('${script}')['prepare']
with tempfile.TemporaryDirectory(prefix='aetheria-kingdom-temple-reject-') as directory:
    ...
    shutil.copytree('scripts/art_sources/monsters/v18/masters',base/'drift')
    with (base/'drift'/'kingdom-knight.png').open('ab') as f:f.write(b'drift')
