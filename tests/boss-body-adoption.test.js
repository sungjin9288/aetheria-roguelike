...
const base = new URL('../scripts/art_sources/monsters/v20/', import.meta.url);
...
const bodies = {
    '공허의 심판자': 'void-judge',
    '균열의 사령관': 'rift-commander',
    '멸절의 사도': 'annihilation-apostle',
    '심연의 파수꾼': 'abyss-sentinel',
    '원한의 용사': 'resentful-champion',
    '전초기지 사령관': 'outpost-commander',
    '종말의 기사': 'doom-knight',
    '차원 분열자': 'dimension-splitter',
    '허무의 전령': 'void-herald',
};

test('nine boss portraits resolve to the reviewed body bytes through authored registry and runtime', async () => {
    ...
    for (const [name, slug] of Object.entries(bodies)) {
        ...
        assert.ok(correction, `${name} must not use a decorated ghost-knight prototype`);
...
test('historical first-nine checkpoint changed only nine pins before the approved next-eight adoption', async () => {
    const before = await readJson(new URL('previous/monsterArtManifest.json', base));
    // The next batch preserves this checkpoint; its adoption test owns current 246-body preservation.
    const successor = new URL('scripts/art_sources/monsters/v21/previous/', root);
    ...
    const nextBodies = new Set(['공허의 대행자', '무한의 화신', '에테르 심판자', '봄의 여왕',
    ...
    for (const [name, entry] of Object.entries(oldEntries)) {
        ...
        if (Object.hasOwn(bodies, name)) {
            ...
            const preserved = nextBodies.has(name)
                ? new URL(`${entry.key}.png`, successor)
                : finalBodies.has(name)
                    ? new URL(`scripts/art_sources/monsters/v22/previous/${entry.key}.png`, root)
    ...
    assert.equal(Object.keys(oldEntries).length - changed.length, 245);
