...
const script = 'scripts/art_sources/monsters/v16/prepare.py';
function python(body) {
    assert.ok(existsSync(script), 'reviewed void-chaos source preparer is required');
    const result = spawnSync('python3', ['-c', body], { encoding: 'utf8' });
    assert.equal(result.status, 0, result.stderr);
}

test('void-chaos exports preserve originals and aspect ratio with deterministic bounded binary alpha', () => {
    python(`
import hashlib, json, runpy, tempfile
from pathlib import Path
from PIL import Image
prepare = runpy.run_path('${script}')['prepare']
source = Path('scripts/art_sources/monsters/v16/masters')
before = {p.name:p.read_bytes() for p in source.glob('*.png')}
names = {'abyss-eye','dimensional-infantry','dragon-knight','fallen-angel','void-beast','void-bombardier','void-fragment'}
with tempfile.TemporaryDirectory(prefix='aetheria-void-chaos-test-') as directory:
    base=Path(directory)
...
test('void-chaos preparation refuses existing output and source drift before creating files', () => {
    python(`
import runpy, shutil, tempfile
from pathlib import Path
prepare=runpy.run_path('${script}')['prepare']
with tempfile.TemporaryDirectory(prefix='aetheria-void-chaos-reject-') as directory:
    ...
    shutil.copytree('scripts/art_sources/monsters/v16/masters',base/'drift')
    with (base/'drift'/'abyss-eye.png').open('ab') as f:f.write(b'drift')
