...
const script = 'scripts/art_sources/monsters/v22/prepare.py';
function python(body) {
    assert.ok(existsSync(script), 'boss-source-seven preparer must exist');
    const result = spawnSync('python3', ['-c', body], { encoding: 'utf8' });
    assert.equal(result.status, 0, result.stderr);
}

test('boss-source-seven exports preserve seven masters and generate deterministic bounded sprites', () => {
    python(`
import hashlib, json, runpy, tempfile
from pathlib import Path
from PIL import Image
prepare=runpy.run_path('${script}')['prepare']
source=Path('scripts/art_sources/monsters/v22/masters')
names={'void-lord','demon-king','aether-lord','entropy-lord','dimension-demon','void-emperor','aether-dragon'}
before={p.name:p.read_bytes() for p in source.glob('*.png')}
assert set(before)=={name+'.png' for name in names}
with tempfile.TemporaryDirectory(prefix='aetheria-boss-source-seven-') as directory:
    base=Path(directory)
...
test('boss-source-seven preparation rejects every drifted master and existing output before writes', () => {
    python(`
