...
const script = 'scripts/art_sources/monsters/v19/prepare.py';
function python(body) {
    assert.ok(existsSync(script), 'reviewed endpoint source preparer is required');
    const result = spawnSync('python3', ['-c', body], { encoding: 'utf8' });
    assert.equal(result.status, 0, result.stderr);
}

test('endpoint exports preserve originals and aspect ratio with deterministic bounded binary alpha', () => {
    python(`
import hashlib, json, runpy, tempfile
from pathlib import Path
from PIL import Image
prepare = runpy.run_path('${script}')['prepare']
source = Path('scripts/art_sources/monsters/v19/masters')
before = {p.name:p.read_bytes() for p in source.glob('*.png')}
names = {'aether-fragment','dimensional-fissure'}
with tempfile.TemporaryDirectory(prefix='aetheria-endpoint-test-') as directory:
    base=Path(directory)
...
test('endpoint preparation refuses existing output and source drift before creating files', () => {
    python(`
import runpy, shutil, tempfile
from pathlib import Path
prepare=runpy.run_path('${script}')['prepare']
with tempfile.TemporaryDirectory(prefix='aetheria-endpoint-reject-') as directory:
    ...
    shutil.copytree('scripts/art_sources/monsters/v19/masters',base/'drift')
    with (base/'drift'/'aether-fragment.png').open('ab') as f:f.write(b'drift')
