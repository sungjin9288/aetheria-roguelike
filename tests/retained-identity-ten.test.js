...
const script = 'scripts/art_sources/monsters/v26/prepare.py';
function python(body) {
    assert.ok(existsSync(script), 'retained-ten preparer must exist');
    const result = spawnSync('python3', ['-c', body], { encoding: 'utf8' });
    assert.equal(result.status, 0, result.stderr);
}

test('retained ten preserves masters and exports deterministic bounded RGBA sprites', () => {
    python(`
import hashlib, json, runpy, tempfile
from pathlib import Path
from PIL import Image
prepare=runpy.run_path('${script}')['prepare']
source=Path('scripts/art_sources/monsters/v26/masters')
names={'current-tracker','mutant-specimen','aether-residue','aether-absorber','dead-priest','dark-priest','undead-mage','fire-lizard','flame-lizard','volcanic-lizard'}
before={p.name:p.read_bytes() for p in source.glob('*.png')}
assert set(before)=={name+'.png' for name in names}
with tempfile.TemporaryDirectory(prefix='aetheria-retained-ten-') as directory:
    base=Path(directory)
...
test('retained ten rejects source drift and existing outputs before writes', () => {
    python(`
import runpy, shutil, tempfile
from pathlib import Path
prepare=runpy.run_path('${script}')['prepare']
source=Path('scripts/art_sources/monsters/v26/masters')
with tempfile.TemporaryDirectory(prefix='aetheria-retained-ten-reject-') as directory:
...
test('retained ten rejects opaque or empty candidates without output', () => {
    python(`
...
source=Path('scripts/art_sources/monsters/v26/masters')
with tempfile.TemporaryDirectory(prefix='aetheria-retained-ten-alpha-') as directory:
    base=Path(directory)
    for mode, color, message in [('RGB',(1,2,3),'transparency'),('RGBA',(0,0,0,0),'Empty silhouette')]:
        ...
        path=candidate/'volcanic-lizard.png'; Image.new(mode,(16,16),color).save(path)
        original=module['PINS']['volcanic-lizard']
        module['PINS']['volcanic-lizard']=hashlib.sha256(path.read_bytes()).hexdigest()
        ...
        finally: module['PINS']['volcanic-lizard']=original
