...
const script = 'scripts/art_sources/monsters/v11/prepare.py';
function runPython(body) {
    assert.ok(existsSync(script), 'source-pinned aquatic preparation is required');
    const result = spawnSync('python3', ['-c', body], { encoding: 'utf8' });
    assert.equal(result.status, 0, result.stderr);
}

test('six aquatic exports preserve masters, aspect ratio, binary alpha and deterministic bytes', () => {
    runPython(`
import hashlib, json, runpy, tempfile
from pathlib import Path
from PIL import Image
prepare = runpy.run_path('${script}')['prepare']
source = Path('scripts/art_sources/monsters/v11/masters')
before = {p.name: p.read_bytes() for p in source.glob('*.png')}
names = {'oblivion-naga','abyss-kraken','deep-sea-prayer','abyss-leviathan','resentful-dead','current-sentinel'}
with tempfile.TemporaryDirectory(prefix='aetheria-aquatic-test-') as directory:
    base = Path(directory)
    ...
    assert Image.open(source/'deep-sea-prayer.png').size == (1254,1254)
assert before == {p.name: p.read_bytes() for p in source.glob('*.png')}
`);
});

test('aquatic preparation refuses existing output and source drift before writing any output', () => {
    runPython(`
import runpy, shutil, tempfile
from pathlib import Path
prepare = runpy.run_path('${script}')['prepare']
source = Path('scripts/art_sources/monsters/v11/masters')
with tempfile.TemporaryDirectory(prefix='aetheria-aquatic-reject-') as directory:
    base = Path(directory)
    ...
    with (base/'drift'/'deep-sea-prayer.png').open('ab') as file: file.write(b'drift')
    try:
