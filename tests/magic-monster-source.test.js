...
const script = 'scripts/art_sources/monsters/v14/prepare.py';
function python(body) {
    assert.ok(existsSync(script), 'reviewed magic source preparer is required');
    const result = spawnSync('python3', ['-c', body], { encoding: 'utf8' });
    assert.equal(result.status, 0, result.stderr);
}

test('magic exports preserve originals and aspect ratio with deterministic bounded binary alpha', () => {
    python(`
import hashlib, json, runpy, tempfile
from pathlib import Path
from PIL import Image
prepare = runpy.run_path('${script}')['prepare']
source = Path('scripts/art_sources/monsters/v14/masters')
before = {p.name:p.read_bytes() for p in source.glob('*.png')}
names = {'magic-orb','magic-puppet','page-spirit'}
with tempfile.TemporaryDirectory(prefix='aetheria-magic-test-') as directory:
    base=Path(directory)
...
test('magic preparation refuses existing output and source drift before creating files', () => {
    python(`
import runpy, shutil, tempfile
from pathlib import Path
prepare=runpy.run_path('${script}')['prepare']
with tempfile.TemporaryDirectory(prefix='aetheria-magic-reject-') as directory:
    ...
    shutil.copytree('scripts/art_sources/monsters/v14/masters',base/'drift')
    with (base/'drift'/'magic-orb.png').open('ab') as f:f.write(b'drift')
