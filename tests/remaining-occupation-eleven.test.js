...
const script = 'scripts/art_sources/monsters/v27/occupation/prepare.py';
function python(body) {
    assert.ok(existsSync(script), 'remaining-occupation-eleven preparer must exist');
    const result = spawnSync('python3', ['-c', body], { encoding: 'utf8' });
    assert.equal(result.status, 0, result.stderr);
}

test('remaining occupation eleven preserves masters and exports deterministic bounded RGBA sprites', () => {
    python(`
import hashlib, json, runpy, tempfile
from pathlib import Path
from PIL import Image
prepare=runpy.run_path('${script}')['prepare']
source=Path('scripts/art_sources/monsters/v27/occupation/masters')
names={'river-spirit','torturer','shadow-hunter','stormcloud-hunter','wind-tracker','treasure-hunter','bone-collector','desert-bandit','cursed-fisher','greedy-merchant','soul-draining-skeleton'}
before={p.name:p.read_bytes() for p in source.glob('*.png')}
assert set(before)=={name+'.png' for name in names}
with tempfile.TemporaryDirectory(prefix='aetheria-remaining-occupation-eleven-') as directory:
    base=Path(directory)
...
test('remaining occupation eleven rejects source drift and existing outputs before writes', () => {
    python(`
import runpy, shutil, tempfile
from pathlib import Path
prepare=runpy.run_path('${script}')['prepare']
source=Path('scripts/art_sources/monsters/v27/occupation/masters')
with tempfile.TemporaryDirectory(prefix='aetheria-remaining-occupation-eleven-reject-') as directory:
...
test('remaining occupation eleven rejects opaque or empty candidates without output', () => {
    python(`
...
source=Path('scripts/art_sources/monsters/v27/occupation/masters')
with tempfile.TemporaryDirectory(prefix='aetheria-remaining-occupation-eleven-alpha-') as directory:
    base=Path(directory)
    for mode, color, message in [('RGB',(1,2,3),'transparency'),('RGBA',(0,0,0,0),'Empty silhouette')]:
        ...
        path=candidate/'soul-draining-skeleton.png'; Image.new(mode,(16,16),color).save(path)
        original=module['PINS']['soul-draining-skeleton']
        module['PINS']['soul-draining-skeleton']=hashlib.sha256(path.read_bytes()).hexdigest()
        ...
        finally: module['PINS']['soul-draining-skeleton']=original
