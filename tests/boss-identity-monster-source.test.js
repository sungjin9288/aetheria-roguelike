...
const script = 'scripts/art_sources/monsters/v21/prepare.py';
function python(body) {
    assert.ok(existsSync(script), 'boss-identity preparer must exist');
    const result = spawnSync('python3', ['-c', body], { encoding: 'utf8' });
    assert.equal(result.status, 0, result.stderr);
}

test('boss-identity exports preserve eight masters and generate deterministic bounded sprites', () => {
    python(`
import hashlib, json, runpy, tempfile
from pathlib import Path
from PIL import Image
prepare=runpy.run_path('${script}')['prepare']
source=Path('scripts/art_sources/monsters/v21/masters')
names={'void-proxy','infinite-avatar','aether-judge','spring-queen','guardian-apostle','aether-giant','sewer-queen','chaos-guardian'}
before={p.name:p.read_bytes() for p in source.glob('*.png')}
assert set(before)=={name+'.png' for name in names}
with tempfile.TemporaryDirectory(prefix='aetheria-boss-identity-') as directory:
    base=Path(directory)
...
test('boss-identity preparation rejects every drifted master and existing output before writes', () => {
    python(`
