...
const script = 'scripts/art_sources/monsters/v27/knight-form/prepare.py';
function python(body) {
    assert.ok(existsSync(script), 'remaining-knight-eleven preparer must exist');
    const result = spawnSync('python3', ['-c', body], { encoding: 'utf8' });
    assert.equal(result.status, 0, result.stderr);
}

test('remaining knight eleven preserves masters and exports deterministic bounded RGBA sprites', () => {
    python(`
import hashlib, json, runpy, tempfile
from pathlib import Path
from PIL import Image
prepare=runpy.run_path('${script}')['prepare']
source=Path('scripts/art_sources/monsters/v27/knight-form/masters')
names={'death-knight','wraith-knight','wraith-commander','ghost-knight','drowned-knight','cursed-knight','fallen-knight','fallen-champion','doom-knight','hell-gatekeeper','void-knight'}
before={p.name:p.read_bytes() for p in source.glob('*.png')}
assert set(before)=={name+'.png' for name in names}
with tempfile.TemporaryDirectory(prefix='aetheria-remaining-knight-eleven-') as directory:
    base=Path(directory)
...
test('remaining knight eleven rejects source drift and existing outputs before writes', () => {
    python(`
import runpy, shutil, tempfile
from pathlib import Path
prepare=runpy.run_path('${script}')['prepare']
source=Path('scripts/art_sources/monsters/v27/knight-form/masters')
with tempfile.TemporaryDirectory(prefix='aetheria-remaining-knight-eleven-reject-') as directory:
...
test('remaining knight eleven rejects opaque or empty candidates without output', () => {
    python(`
...
source=Path('scripts/art_sources/monsters/v27/knight-form/masters')
with tempfile.TemporaryDirectory(prefix='aetheria-remaining-knight-eleven-alpha-') as directory:
    base=Path(directory)
    for mode, color, message in [('RGB',(1,2,3),'transparency'),('RGBA',(0,0,0,0),'Empty silhouette')]:
        ...
        path=candidate/'void-knight.png'; Image.new(mode,(16,16),color).save(path)
        original=module['PINS']['void-knight']
        module['PINS']['void-knight']=hashlib.sha256(path.read_bytes()).hexdigest()
        ...
        finally: module['PINS']['void-knight']=original
