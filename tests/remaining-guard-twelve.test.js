...
const script = 'scripts/art_sources/monsters/v27/guard-duty/prepare.py';
function python(body) {
    assert.ok(existsSync(script), 'remaining-guard-twelve preparer must exist');
    const result = spawnSync('python3', ['-c', body], { encoding: 'utf8' });
    assert.equal(result.status, 0, result.stderr);
}

test('remaining guard twelve preserves masters and exports deterministic bounded RGBA sprites', () => {
    python(`
import hashlib, json, runpy, tempfile
from pathlib import Path
from PIL import Image
prepare=runpy.run_path('${script}')['prepare']
source=Path('scripts/art_sources/monsters/v27/guard-duty/masters')
names={'arcane-watcher','rift-sentinel','temple-guard','laboratory-guardian','abyss-guardian','darkness-guardian','outpost-watchman','last-guardian','tower-guardian','storm-guardian','trap-guardian','crystal-keeper'}
before={p.name:p.read_bytes() for p in source.glob('*.png')}
assert set(before)=={name+'.png' for name in names}
with tempfile.TemporaryDirectory(prefix='aetheria-remaining-guard-twelve-') as directory:
    base=Path(directory)
...
test('remaining guard twelve rejects source drift and existing outputs before writes', () => {
    python(`
import runpy, shutil, tempfile
from pathlib import Path
prepare=runpy.run_path('${script}')['prepare']
source=Path('scripts/art_sources/monsters/v27/guard-duty/masters')
with tempfile.TemporaryDirectory(prefix='aetheria-remaining-guard-twelve-reject-') as directory:
...
test('remaining guard twelve rejects opaque or empty candidates without output', () => {
    python(`
...
source=Path('scripts/art_sources/monsters/v27/guard-duty/masters')
with tempfile.TemporaryDirectory(prefix='aetheria-remaining-guard-twelve-alpha-') as directory:
    base=Path(directory)
    for mode, color, message in [('RGB',(1,2,3),'transparency'),('RGBA',(0,0,0,0),'Empty silhouette')]:
        ...
        path=candidate/'crystal-keeper.png'; Image.new(mode,(16,16),color).save(path)
        original=module['PINS']['crystal-keeper']
        module['PINS']['crystal-keeper']=hashlib.sha256(path.read_bytes()).hexdigest()
        ...
        finally: module['PINS']['crystal-keeper']=original
