...
const script = 'scripts/art_sources/monsters/v27/caster-role/prepare.py';
function python(body) {
    assert.ok(existsSync(script), 'remaining-caster-seven preparer must exist');
    const result = spawnSync('python3', ['-c', body], { encoding: 'utf8' });
    assert.equal(result.status, 0, result.stderr);
}

test('remaining caster seven preserves masters and exports deterministic bounded RGBA sprites', () => {
    python(`
import hashlib, json, runpy, tempfile
from pathlib import Path
from PIL import Image
prepare=runpy.run_path('${script}')['prepare']
source=Path('scripts/art_sources/monsters/v27/caster-role/masters')
names={'ancient-mage','void-mage','enraged-magus','trickster-mage','apocalypse-mage','doomsday-herald','flame-priest'}
before={p.name:p.read_bytes() for p in source.glob('*.png')}
assert set(before)=={name+'.png' for name in names}
with tempfile.TemporaryDirectory(prefix='aetheria-remaining-caster-seven-') as directory:
    base=Path(directory)
...
test('remaining caster seven rejects source drift and existing outputs before writes', () => {
    python(`
import runpy, shutil, tempfile
from pathlib import Path
prepare=runpy.run_path('${script}')['prepare']
source=Path('scripts/art_sources/monsters/v27/caster-role/masters')
with tempfile.TemporaryDirectory(prefix='aetheria-remaining-caster-seven-reject-') as directory:
...
test('remaining caster seven rejects opaque or empty candidates without output', () => {
    python(`
...
source=Path('scripts/art_sources/monsters/v27/caster-role/masters')
with tempfile.TemporaryDirectory(prefix='aetheria-remaining-caster-seven-alpha-') as directory:
    base=Path(directory)
    for mode, color, message in [('RGB',(1,2,3),'transparency'),('RGBA',(0,0,0,0),'Empty silhouette')]:
        ...
        path=candidate/'void-mage.png'; Image.new(mode,(16,16),color).save(path)
        original=module['PINS']['void-mage']
        module['PINS']['void-mage']=hashlib.sha256(path.read_bytes()).hexdigest()
        ...
        finally: module['PINS']['void-mage']=original
