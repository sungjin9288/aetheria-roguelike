...
const script = 'scripts/art_sources/monsters/v27/stone-material/prepare.py';
function python(body) {
    assert.ok(existsSync(script), 'remaining-stone-six preparer must exist');
    const result = spawnSync('python3', ['-c', body], { encoding: 'utf8' });
    assert.equal(result.status, 0, result.stderr);
}

test('remaining stone six preserves masters and exports deterministic bounded RGBA sprites', () => {
    python(`
import hashlib, json, runpy, tempfile
from pathlib import Path
from PIL import Image
prepare=runpy.run_path('${script}')['prepare']
source=Path('scripts/art_sources/monsters/v27/stone-material/masters')
names={'prison-golem','ore-golem','stone-giant','statue-guardian','eternal-idol','gold-golem'}
before={p.name:p.read_bytes() for p in source.glob('*.png')}
assert set(before)=={name+'.png' for name in names}
with tempfile.TemporaryDirectory(prefix='aetheria-remaining-stone-six-') as directory:
    base=Path(directory)
...
test('remaining stone six rejects source drift and existing outputs before writes', () => {
    python(`
import runpy, shutil, tempfile
from pathlib import Path
prepare=runpy.run_path('${script}')['prepare']
source=Path('scripts/art_sources/monsters/v27/stone-material/masters')
with tempfile.TemporaryDirectory(prefix='aetheria-remaining-stone-six-reject-') as directory:
...
test('remaining stone six rejects opaque or empty candidates without output', () => {
    python(`
...
source=Path('scripts/art_sources/monsters/v27/stone-material/masters')
with tempfile.TemporaryDirectory(prefix='aetheria-remaining-stone-six-alpha-') as directory:
    base=Path(directory)
    for mode, color, message in [('RGB',(1,2,3),'transparency'),('RGBA',(0,0,0,0),'Empty silhouette')]:
        ...
        path=candidate/'gold-golem.png'; Image.new(mode,(16,16),color).save(path)
        original=module['PINS']['gold-golem']
        module['PINS']['gold-golem']=hashlib.sha256(path.read_bytes()).hexdigest()
        ...
        finally: module['PINS']['gold-golem']=original
