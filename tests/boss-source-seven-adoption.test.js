...
const base = new URL('../scripts/art_sources/monsters/v22/', import.meta.url);
...
const bodies = {
    ...
    "마왕": "demon-king",
    "에테르 군주": "aether-lord",
    "엔트로피 군주": "entropy-lord",
    "차원 마왕": "dimension-demon",
    "허무의 황제": "void-emperor",
    "에테르 드래곤": "aether-dragon"
};

test('seven boss portraits resolve to the reviewed body bytes through authored registry and runtime', async () => {
...
test('boss checkpoint preserves seven source pins before the approved regional-three adoption', async () => {
    const before = await readJson(new URL('previous/monsterArtManifest.json', base));
    const successor = new URL('scripts/art_sources/monsters/v23/previous/', root);
    ...
    for (const [name, entry] of Object.entries(oldEntries)) {
        ...
        if (Object.hasOwn(bodies, name)) {
            ...
            const preserved = regionalBodies.has(name)
    ...
    assert.equal(Object.keys(oldEntries).length - changed.length, 247);
