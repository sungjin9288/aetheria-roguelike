...
const script = 'scripts/art_sources/monsters/v23/prepare.py';
function python(body) {
    assert.ok(existsSync(script), 'regional-identity-three preparer must exist');
    const result = spawnSync('python3', ['-c', body], { encoding: 'utf8' });
    assert.equal(result.status, 0, result.stderr);
}

test('regional-identity-three exports preserve three masters and generate deterministic bounded sprites', () => {
    python(`
import hashlib, json, runpy, tempfile
from pathlib import Path
from PIL import Image
prepare=runpy.run_path('${script}')['prepare']
source=Path('scripts/art_sources/monsters/v23/masters')
names={'frost-wolf','ink-slime','flame-watcher'}
before={p.name:p.read_bytes() for p in source.glob('*.png')}
assert set(before)=={name+'.png' for name in names}
with tempfile.TemporaryDirectory(prefix='aetheria-regional-identity-three-') as directory:
    base=Path(directory)
...
test('regional-identity-three preparation rejects every drifted master and existing output before writes', () => {
    python(`
