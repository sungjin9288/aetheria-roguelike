                    ...
                    <div className="bg-black/18 border border-white/8 rounded-[1rem] p-3 space-y-2">
                        ...
                        {m.bossBrief && (
                            <div className="rounded-[0.95rem] border border-rose-300/20 bg-rose-400/[0.08] px-2.5 py-2 text-[10px] font-fira text-slate-300 space-y-1.5">
                                ...
                                {/* cycle 245: warningChips / recommendedBuilds dispatch — BOSS_BRIEFS 데이터 정의 후 UI 표시 0건이던 dead config fix. */}
                                {Array.isArray(m.bossBrief.warningChips) && m.bossBrief.warningChips.length > 0 && (
                                    <div className="flex flex-wrap items-center gap-1 pt-0.5">
                                        ...
                                        {m.bossBrief.warningChips.map((chip: string) => (
                                            <span key={chip} className="rounded-md bg-rose-400/15 border border-rose-300/30 px-1.5 py-0.5 text-[9px] text-rose-100">{chip}</span>
                                        ))}
                                    </div>
                                )}
                                {Array.isArray(m.bossBrief.recommendedBuilds) && m.bossBrief.recommendedBuilds.length > 0 && (
                                    <div className="flex flex-wrap items-center gap-1 pt-0.5">
                                        ...
                                        {m.bossBrief.recommendedBuilds.map((build: string) => (
                                            <span key={build} className="rounded-md bg-emerald-400/15 border border-emerald-300/30 px-1.5 py-0.5 text-[9px] text-emerald-100">{build}</span>
                                        ))}
                                    </div>
                                )}
                            </div>
