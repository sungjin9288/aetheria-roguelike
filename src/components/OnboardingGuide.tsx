...
const STEPS: any = [
    { id: 'explore', label: '① EXPLORE', desc: '탐색으로 첫 전투 시작', color: 'cyber-blue', done: (s: any) => s.kills > 0 || (s.explores || 0) > 0 },
    { id: 'move', label: '② MOVE', desc: '다른 지역으로 이동해보세요', color: 'cyber-green', done: (s: any, loc: any) => loc !== '시작의 마을' },
...
const OnboardingGuide = ({ player, onDismiss }: OnboardingGuideProps) => {
    ...
    const activeStep = STEPS.findIndex((s: any) => !s.done(player.stats || {}, player.loc));
