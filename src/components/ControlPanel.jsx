...
/**
 * ControlPanel - Game action controls
 * v3.6: Refactored to use separated ShopPanel and EventPanel components
 */
const ControlPanel = ({ gameState, player, actions, setGameState, shopItems, grave, isAiThinking, currentEvent }) => {
    ...
    // EVENT MODE - Choice (using EventPanel component)
    if (gameState === 'event' && !isAiThinking) {
        return <EventPanel currentEvent={currentEvent} actions={actions} />;
    }

    // SHOP MODE (using ShopPanel component)
    if (gameState === 'shop') {
        return (
            <ShopPanel
                player={player}
                ...
                <div className="grid grid-cols-4 gap-2">
                    ...
                    {mapData.type === 'safe' && (
                        <>
                            ...
                            >
                                <ScrollText size={16} /> <span className="text-[10px]">의뢰</span>
                            </button>
                            <button
                                disabled={isAiThinking}
                    ...
                    {grave && grave.loc === player.loc && (
                        ...
                        >
                            <Ghost size={16} /> <span className="text-[10px]">유해수습</span>
                        </button>
                    )}
                    <button
                        disabled={isAiThinking}
