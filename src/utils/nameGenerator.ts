...
const pick = (items, rng) => items[randomIndex(items.length, rng)];
...
const trimToFourSyllables = (name) => Array.from(name).slice(0, 4).join('');

const generateFromParts = (rng) => {
    const roll = rng();
...
export const createRandomMobileName = (rng = Math.random) => {
    const roll = rng();
