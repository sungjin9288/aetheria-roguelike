...
export interface SignatureDropSource {
    monster: string;
    rate: number;
}

const buildSourceIndex = (): Record<string, SignatureDropSource[]> => {
    const index: Record<string, SignatureDropSource[]> = {};
 ...
 *
 * @param itemName 미등록/null → []
 */
export const getSignatureDropSources = (itemName: string | null | undefined): ReadonlyArray<SignatureDropSource> => {
 ...
 */
export const getAllSignatureDropSourceIndex = (): Readonly<Record<string, ReadonlyArray<SignatureDropSource>>> => SOURCE_INDEX;
