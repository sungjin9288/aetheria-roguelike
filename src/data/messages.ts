...
export const MSG = {
    ...
    ACH_DONE: (title: string) => `업적 달성: ${title}`,
    ...
    MOVE_LEVEL_REQUIRED: (lv: number) => `레벨 ${lv} 이상에서 입장할 수 있습니다.`,
    ...
    QUEST_LEVEL_REQUIRED: (lv: number) => `레벨 ${lv} 이상부터 수락할 수 있는 퀘스트입니다.`,
    QUEST_ACCEPTED: (title: string) => `퀘스트 수락: ${title}`,
    ...
    MOVE_AREA_DANGER: (lv: number) => `⚠️ 권장 레벨 ${lv} 지역입니다. 아직 이곳은 당신에게 벅찹니다 — 정예와 구역 보스를 경계하고, 위태로우면 후퇴하십시오.`,
