...
for start in range(0, len(names), 3):
    ...
    with (BASE / f'review-{start // 3 + 1}.png').open('xb') as target:
        sheet.save(target)
gray = Image.new('RGB', (600, 480), '#f3ece0')
draw = ImageDraw.Draw(gray)
for index, name in enumerate(names):
    sprite = Image.open(BASE / 'exports' / f'{name}.png')
...
with (BASE / 'review-grayscale.png').open('xb') as target:
    gray.save(target)
print('12 pinned candidates rendered in color and grayscale; masters and runtime unchanged')
