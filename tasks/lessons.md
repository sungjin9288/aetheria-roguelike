...
- **Rule:** 모바일 목록이 비어 있으면 제목, 수량, 빈 안내를 각각 별도 영역으로 쌓지 않는다. 한 줄 상태로 합쳐 실제로 선택할 수 있는 다음 행동을 같은 화면에 올리고, 항목이 생겼을 때만 상세 목록 구조를 복원한다
- **Rationale:** 빈 상태는 정보가 아니라 부재를 알리는 신호다. 큰 빈 카드는 사용 가능한 기능을 아래로 밀어 탐색 비용만 늘리므로, 데이터가 없을수록 화면도 짧아져야 한다
